...
define('SuperProcessName', 'frame-Controller');
...
if (CheckProcessExist()) { //如果存在 说明已经运行了 则通过unixsock通信
            ...
            if ($cmd == 'status') {
                if (empty($ret['data'])) {
                    ...
                    foreach ($ret['data'] as $single) {
                        echo 'Server Name is ' . "\033[32;40m " . $single['name'] . " \033[0m" . '  ' . PHP_EOL . '--php start path is ' . $single['php'] . '  ' . PHP_EOL . '--start time is ' . $single['runTime'] . PHP_EOL;
                    }
                }
            } else {
    ...
    if ($cmd == 'start') {
        ...
        $process = new swoole_process(function (swoole_process $worker) use ($name, $cmd, $phpStart) {//目前指支持一个
            $worker->exec($phpStart, array(STARTSHELLPATH, $cmd, $name));//拉起server
        }, false);
        $pid = $process->start();
        ...
        $RunningServer[$name] = array('php' => $phpStart, 'name' => $name, 'runTime' => date('Y-m-d H:i:s'));
    ...
    $serv->on('receive', function ($serv, $fd, $from_id, $data) {
        ...
        if ($opData['cmd'] == 'start') { //添加到runserver  还是需要获取路径 存入数组中
            ...
            if ($retConf['r'] != 0) { //
                ...
                StartLog(__LINE__ . " $phpStart " . STARTSHELLPATH . '  ' . $opData['cmd'] . ' ' . $opData['server']);
                $serv->runServer[$opData['server']] = array('php' => $phpStart, 'name' => $opData['server'], 'runTime' => date('Y-m-d H:i:s')); //添加到runServer中
            ...
            if (!isset($serv->runServer[$opData['server']])) { //如果已经有了，说明服务已经启动
                $serv->send($fd, json_encode(array('r' => 1, "msg" => $opData['server'] . ' is not running')));
                StartLog(__LINE__ . 'receive data is' . json_encode(array('r' => 1, "msg" => $opData['server'] . ' is not running')));
                return;
            };
            ...
            StartServ($phpStart, 'stop', $opData['server']);
            StartLog(__LINE__ . "$phpStart " . STARTSHELLPATH . '  ' . ' stop ' . $opData['server']);
            sleep(1);
            //   exec("$phpStart ".STARTBASEPATH . "/lib/Swoole/shell/start.php ".' start '.$opData['server']);//
            StartServ($phpStart, 'start', $opData['server']);
            StartLog(__LINE__ . "$phpStart " . STARTSHELLPATH . '  ' . ' start ' . $opData['server']);
            $serv->runServer[$opData['server']] = array('php' => $phpStart, 'name' => $opData['server'], 'runTime' => date('Y-m-d H:i:s')); //添加到runServer中
...
{
    error_log($msg . PHP_EOL, 3, '/data/log/SuperMaster.log');
}

function StartLogTimer($msg)
{
    error_log($msg . PHP_EOL, 3, '/data/log/SuperMasterTimer.log');
}
...
function sendCmdToServ($data)
{
    ...
    StartLog(__LINE__ . ' sendCmdToServ ret is' . print_r($data, true));
...
function printInfo()
{
    echo "welcome to use frame-Controller,we can help you to monitor your swoole server!" . PHP_EOL;
    echo "please input server name and cmd: \033[31;40m [./frame myServerName start] \033[0m  " . PHP_EOL;
    echo "support cmds: \033[31;40m [start stop reload restart status startall list] \033[0m" . PHP_EOL;
    echo "if you want to stop Swoole-Controller please input :  \033[31;40m [./frame shutdown] \033[0m " . PHP_EOL;
    echo "if you want to know running servername please input : \033[31;40m [./frame status] \033[0m " . PHP_EOL;
    echo "if you want to know server list that you can start please input : \033[31;40m [./frame list] \033[0m " . PHP_EOL;
    echo "if you want to start all your servers please input : \033[31;40m [./frame startall] \033[0m " . PHP_EOL;
    exit;
}